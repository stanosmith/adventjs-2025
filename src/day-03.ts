...
export function drawGift(size: number, symbol: string): string {
	...
	const fillRow = (_none: string, index: number) => {
		if (index === 0 || index === size - 1) return symbol;
		return " ";
	};

	return createRow()
		.map((_line, index) => {
			if (index === 0 || index === size - 1) return createRow(symbol);
			return createRow().map(fillRow);
		})
		.map((line) => line.join(""))
		.join("\n");

	function createRow(symbol?: string): string[] {
		return Array(size).fill(symbol ?? "");
	}
}
